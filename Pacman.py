...
PACMAN_ALL_IMAGES_FILEPATHS = [PACMAN_1_IMG_FILEPATH, PACMAN_2_IMG_FILEPATH, PACMAN_3_IMG_FILEPATH,
                                 PACMAN_4_IMG_FILEPATH, PACMAN_5_IMG_FILEPATH]
# Sounds
...
movement = 2
# Pacman Images
pacmanImages = []  # Will hold Pacman Imgs resources
...
# Loads Pacman images into pacmanImages (list[pygame.Surface])
def loadPacmanImages():
    for imagePath in PACMAN_ALL_IMAGES_FILEPATHS:
        pacmanImages.append(pygame.image.load(imagePath))

def movementPacman(pacman, direccion):
...
def mapa(ventana):
    ...
    pygame.draw.line(ventana, (0, 0, randBlue), (0, 599), (799, 599), 2)
    pygame.draw.line(ventana, (0, 0, randBlue), (3, 123), (796, 123), 2)
    pygame.draw.line(ventana, (0, 0, randBlue), (3, 596), (796, 596), 2)
    # Verticales
...
# Creates the dots coordinates in a list
# :param filePath: string filepath that states the coords having a coord per line separated by ',' (single comma)
# :return:
# :rtype: list[list[int]]
def createDotCoordinatesList() -> list[list[int]]:
    dotCoords = []
    file = open(DOT_COORDS_FILEPATH, "r", encoding="UTF-8")
    line = file.readline()
    while line != "":
...
def createDotsSprites(listaDots, dotCoordList: list[list[int]]):
...
def renderDotsInWindow(ventana, listaDots):
    for dot in listaDots:
        ventana.blit(dot.image, dot.rect)

def checkScore(pacman, listaDots):
    for dotIndex in range(-1, -len(listaDots) - 1, -1):
        if pacman.rect.colliderect(listaDots[dotIndex].rect):
            dot = listaDots[dotIndex]
            listaDots.remove(dot)
            return 10
    return 0

def movementFantasmas(phantoms, ticks) -> list[list[pygame.sprite.Sprite, int]]:
    fanA = phantoms[0][0]
    fanB = phantoms[1][0]
    fanC = phantoms[2][0]
    fanD = phantoms[3][0]
    if fanA.rect.left < 385:
        fanA.rect.left += movement
    if ticks >= 180:
        if fanB.rect.left > 385:
            fanB.rect.left -= movement
    if ticks >= 360:
        if fanC.rect.left < 385:
            fanC.rect.left += movement
    if ticks >= 540:
        if fanD.rect.left > 385:
            fanD.rect.left -= movement
    ticks += 1
    ...
    for index in range(len(phantoms)):
        ...
        if 129 < phantom.rect.left < 385 and 285 < phantom.rect.top < 405:
            if phantomRandomDirection == 1:
                phantom.rect.top -= movement
            elif phantomRandomDirection == 2:
                phantom.rect.left -= movement
...
def dibujar():
    ...
    listaDots = []
    createDotsSprites(listaDots, createDotCoordinatesList())
    ticksFan = 0
    ...
    while not termina:  # Ciclo principal
        ...
        if ModoDeJuego == Menu:
            ...
            pacman.image = pacmanImages[animationCounterPacman // 2]
            pacman, direccion = movementPacman(pacman, direccion)
            if ticks <= 650:
                phantoms_List = movementFantasmas(phantoms_List, ticks)
            ...
            ventana.blit(Font.render("Pacman", False, WHITE), (5, 10))
            # ventana.blit(Font.render("%03d, %03d" % (pacman.rect.left, pacman.rect.top), False, WHITE), (5, 10))
            mapa(ventana)
            renderDotsInWindow(ventana, listaDots)
            ...
            scoreTitulo = Font.render("SCORE", False, WHITE)
            scoreNum = Font.render("%04d" % score, False, WHITE)
            score += checkScore(pacman, listaDots)
            ...
            ventana.blit(fondo.image, fondo.rect)
            ventana.blit(Font.render("Perdiste", False, WHITE),
            ...
            ventana.blit(fondo.image, fondo.rect)
            ventana.blit(Font.render("Ganaste", False, WHITE),
